    ...
    switch ($action) {
            ...
            if (!empty($roleId)) {
                if ($csrf && $user_roles->validate_update($postdata)) {
                    if (user_can('edit_role')) {
                        // Disable all existing permissions for this role
                        $permissions->query("UPDATE permission_roles SET disabled = 1 WHERE role_id = ?", [$roleId]);

                        // Re-enable or insert updated permissions
                        ...
                        $user_roles->update_role($roleId, $postdata);
        ...
        case 'add':
            if (!empty($addRole)) {
                ...
                unset($postdata['role_id'], $postdata['id']);

                if ($csrf && $user_roles->validate_insert($postdata)) {
                    if (user_can('add_role')) {
                        if ($user_roles->insert($postdata)) {
                            $newRoleId = $user_roles->insert_id;

                           $perm_data = [
                                'role_id' => $newRoleId,
                                'permission' => '', // optional default or skip
                                'disabled' => 1,
                            ];

                            $permissions->insert($perm_data);
