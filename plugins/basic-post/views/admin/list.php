...
<div class="container-fluid mt-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <p class="text-muted mb-0">Manage your posts</p>
        </div>
        <div class="d-flex gap-2">
            <?php if(user_can('manage_post_layout')): ?>
                <a href="<?= ROOT ?>/<?= $admin_route ?>/<?= $plugin_route ?>/layout" class="btn btn-outline-primary">
    ...
    <div class="card mb-4">
        <div class="card-body">
            <form class="row g-3" method="get" action="">
                <div class="col-md-4">
                    <label for="posts-search" class="form-label visually-hidden">Search</label>
                    <input type="text" id="posts-search" name="search" class="form-control" placeholder="Search by title or description..." value="<?= esc($_GET['search'] ?? '') ?>">
                </div>
                <div class="col-md-2">
            ...
            <div class="table-responsive">
                <table class="table table-hover table-striped mb-0">
                    <thead class="table-dark">
                        <tr>
                            <?php if(user_can('edit_post') || user_can('delete_post')): ?>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <div class="btn-group btn-group-sm" role="group">
                                            <!-- View Live -->
                                            <?php if (empty($row->disabled)): ?>
                                                <a href="<?= ROOT ?>/<?= esc($row->slug) ?>" target="_blank" class="btn btn-outline-primary" title="View Live Post">
                                                    <i class="bi bi-eye"></i>
                                                </a>
                                            <?php endif; ?>
                                            ...
                                            <?php if(user_can('edit_post')): ?>
                                                <a href="<?= ROOT ?>/<?= $admin_route ?>/<?= $plugin_route ?>/edit/<?= $row->id ?>" class="btn btn-outline-warning" title="Edit Post">
                                            ...
                                            <?php if(user_can('delete_post')): ?>
                                                <a href="<?= ROOT ?>/<?= $admin_route ?>/<?= $plugin_route ?>/delete/<?= $row->id ?>" class="btn btn-outline-danger" title="Delete Post">
        ...
        <div class="modal-content">
            ...
            <form id="duplicateForm" method="post" action="<?= ROOT ?>/<?= $admin_route ?>/<?= $plugin_route ?>/duplicate">
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label for="duplicateTitle" class="form-label">New Post Title</label>
...
<!-- Bulk Actions -->
<?php if (!empty($rows) && (user_can('edit_post') || user_can('delete_post'))): ?>
...
(function () {
    ...
    if (bulkForm) {
        bulkForm.addEventListener('submit', function (e) {
            ...
            let confirmMessage = `Are you sure you want to ${action} ${count} selected post(s)?`;
            if (action === 'delete') {
                confirmMessage = `This will permanently delete ${count} selected post(s). This action cannot be undone. Continue?`;
            }
        ...
        <div class="col-md-6">
            <div class="card border-danger">
                <div class="card-body text-center">
                    ...
                    <p class="card-text">You don't have permission to view posts.</p>
