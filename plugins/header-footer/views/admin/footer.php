...
<?php
  $savedLayout = setting('footer_layout');
  if (!is_array($savedLayout) || empty($savedLayout['html'])) {
      $savedLayout = [
          'html' => '<div class="hf-footer-wrapper" style="padding:10px 20px;text-align:center;">'
              . '<div class="hf-dynamic-block hf-copyright" style="padding:8px 15px;border:2px dashed #999;background:#f8f9fa;color:#666;font-size:0.85rem;">&copy; {{COPYRIGHT_YEAR}} {{SITE_NAME}}. All rights reserved.</div>'
              . '</div>',
          'css'  => '',
      ];
  }
...
<div class="container-fluid mt-4">
  <div class="d-flex justify-content-between align-items-center mb-3">
    <div>
      <h4 class="mb-0">Edit Footer</h4>
      <small class="text-muted">Design the footer that appears on every page. Drag in "Site Name" or "Copyright Notice" from the Dynamic category to keep those details automatic.</small>
    </div>
    <div>
      <button id="saveLayoutBtn" class="btn btn-primary">Save Footer</button>
...
<script>
  ...
  document.getElementById('resetLayoutBtn').addEventListener('click', function () {
    if (!confirm('Reset the footer to the default layout? This cannot be undone.')) return;
